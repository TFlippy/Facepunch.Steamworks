...
namespace Steamworks.Data
{
	...
	public struct Connection: IEquatable<Connection>
	{
		...
		public unsafe Result SendMessage(byte[] data, SendType sendType = SendType.Reliable)
		{
			fixed (byte* ptr = data)
			{
				return this.SendMessage((IntPtr)ptr, data.Length, sendType);
			}
		}
		...
		public unsafe Result SendMessage(byte[] data, int offset, int length, SendType sendType = SendType.Reliable)
		{
			fixed (byte* ptr = data)
			{
				return this.SendMessage((IntPtr)ptr + offset, length, sendType);
			}
		}
		...
		{
			var connectionStatus = default(ConnectionStatus);
